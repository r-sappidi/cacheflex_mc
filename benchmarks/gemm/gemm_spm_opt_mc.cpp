...
// VL=16 (2048-bit) + 2D-tiled SPM GEMM. This is the multicore variant of
// gemm_spm_opt.cpp. Two changes versus that kernel:
//
//   1. Vector length. At VL=16 one SVE vector holds 64 fp32, so the nc=64
//      register-tile width is exactly ONE vector. The microkernel therefore
//      issues a single spm.ld1w per K-step (reading the whole 64-col B panel
//      row, 256 B = 4 staged 64 B lines) instead of four, and the C tile is
//      kMr accumulators (one vector per row) instead of 4*kMr.
//
//   2. 2D thread partition. The original splits only M (rows) across cores, so
//      every core stages the *entire* B into its private SPM -> B is read P
//      times from the memory system, making multicore off-chip-bandwidth-bound.
//      Here threads form a PR x PC grid over (M, N): each core owns a row band
//      AND a column band, so a given B panel is staged only by the PR cores
//      sharing its columns (B replication drops from P to PR), while A is the
//      replicated-but-cached operand (read by the PC cores sharing its rows).
//      For shapes where N/nc is divisible by a large factor of P this drives B
//      staging traffic toward 1x and lets the per-core SPM win actually scale.
...
constexpr int kVecsPerRow = kNc / 16;  // 64 B SPM lines per staged panel row
...
// number of nc-panels, so the PR x PC grid tiles evenly and B replication
// (== PR) is minimized.
...
// vector from SPM (z16, == 256 B == 4 staged lines) and broadcasts kMr A
// scalars (z20..z23), issuing kMr independent fmlas. B walks the staged panel
// contiguously (256 B per panel row); A walks down its row by one float/step.
static inline void
...
// Single-row tail: 1 C vector in z0 for rows that do not fill a kMr-row tile.
...
// For this thread's (row band, column band) tile: stage each kc x nc panel of B
// into the core-private SPM, then run all owned rows against it in kMr-row
// register tiles.
...
int main(int argc, char **argv)
{
    ...
    std::printf("gemm_spm_opt_mc threads=%d grid=%dx%d(PRxPC) m=%d k=%d n=%d "
                "kc=%d nc=%d spm_way0=%d spm_base_set=%d warmup=%d repeat=%d "
