...
// VL=16, properly-tiled SPM GEMM: 8-row x 128-col register tile = 16 fp32
// accumulators (z0..z15), the same 16-accumulator block that nc=64 gave at
// VL=4. At VL=16 a vector is 64 fp32, so nc=128 = TWO vectors; the microkernel
// loads 2 B vectors + broadcasts 8 A scalars per K-step and issues 16 FMAs
// (8 rows x 2 col-halves). That lifts FMAs/load from 0.8 (the nc=64 mc kernel,
// 4 accumulators, load-bound) to 1.6, making the kernel compute-bound so the
// SPM-vs-coherent comparison reflects datapath/traffic, not miss latency.
//
// Everything else (2D PR x PC grid, B staged into private SPM split along N)
// matches gemm_spm_opt_mc.cpp.
constexpr std::uint64_t kSpmWindowBase = 0x40000000ull;
constexpr int kNc = 128;      // register-tile width in columns (2 vectors @ VL=16)
constexpr int kMr = 8;        // register-tile height in rows
constexpr int kVecsPerRow = kNc / 16;  // 64 B SPM lines per staged panel row (=8)
...
static Options parse_args(int argc, char **argv)
{
    ...
    if (opt.nc != kNc) {
        std::fprintf(stderr, "this kernel fixes nc=%d (register-tile width)\n", kNc);
        std::exit(2);
    }
    if (opt.n % kNc != 0 || opt.k % opt.kc != 0) {
        std::fprintf(stderr, "requires n %% %d == 0 and k %% kc == 0\n", kNc);
        std::exit(2);
    }
    ...
    if (opt.spm_base_set >= 1024 ||
        ...
        std::fprintf(stderr, "panel of %ld lines exceeds SPM ways 0..5\n", lines);
        std::exit(2);
    }
    return opt;
}
...
{
    asm volatile("SPMCP_64_IMM %0, [%1, #0]\n" : : "r"(dst_spm), "r"(src) : "memory");
}

// Accumulate one 8-row x 128-col tile of C over kc K-steps. 16 C accumulators
// (z0..z15: row r occupies z[2r], z[2r+1]). Each step loads 2 B vectors from
// SPM (z16,z17 = the 128-col panel row, 2x256 B) and broadcasts 8 A scalars
// (z18..z25), issuing 16 fmlas. B advances 512 B/step; A by one float/step.
static inline void
spm_kernel_8x128(float *c0, float *c1, float *c2, float *c3, float *c4,
                 float *c5, float *c6, float *c7, const float *a0,
                 const float *a1, const float *a2, const float *a3,
                 const float *a4, const float *a5, const float *a6,
                 const float *a7, std::uint64_t spm, std::uint64_t kc)
{
    asm volatile(
        "ptrue p7.s\n"
        "ld1w {z0.s},  p7/z, [%[c0]]\n"
        ...
        "ld1w {z2.s},  p7/z, [%[c1]]\n"
        "ld1w {z3.s},  p7/z, [%[c1], #1, mul vl]\n"
        "ld1w {z4.s},  p7/z, [%[c2]]\n"
        "ld1w {z5.s},  p7/z, [%[c2], #1, mul vl]\n"
        "ld1w {z6.s},  p7/z, [%[c3]]\n"
        "ld1w {z7.s},  p7/z, [%[c3], #1, mul vl]\n"
        "ld1w {z8.s},  p7/z, [%[c4]]\n"
        "ld1w {z9.s},  p7/z, [%[c4], #1, mul vl]\n"
        "ld1w {z10.s}, p7/z, [%[c5]]\n"
        "ld1w {z11.s}, p7/z, [%[c5], #1, mul vl]\n"
        "ld1w {z12.s}, p7/z, [%[c6]]\n"
        "ld1w {z13.s}, p7/z, [%[c6], #1, mul vl]\n"
        "ld1w {z14.s}, p7/z, [%[c7]]\n"
        "ld1w {z15.s}, p7/z, [%[c7], #1, mul vl]\n"
        "1:\n"
        ...
        "fmla z1.s,  p7/m, z17.s, z18.s\n"
        "fmla z2.s,  p7/m, z16.s, z19.s\n"
        "fmla z3.s,  p7/m, z17.s, z19.s\n"
        "fmla z4.s,  p7/m, z16.s, z20.s\n"
        "fmla z5.s,  p7/m, z17.s, z20.s\n"
        "fmla z6.s,  p7/m, z16.s, z21.s\n"
        "fmla z7.s,  p7/m, z17.s, z21.s\n"
        "fmla z8.s,  p7/m, z16.s, z22.s\n"
        "fmla z9.s,  p7/m, z17.s, z22.s\n"
        "fmla z10.s, p7/m, z16.s, z23.s\n"
        "fmla z11.s, p7/m, z17.s, z23.s\n"
        "fmla z12.s, p7/m, z16.s, z24.s\n"
        "fmla z13.s, p7/m, z17.s, z24.s\n"
        "fmla z14.s, p7/m, z16.s, z25.s\n"
        "fmla z15.s, p7/m, z17.s, z25.s\n"
        "subs %[k], %[k], #1\n"
        "b.ne 1b\n"
        "st1w {z0.s},  p7, [%[c0]]\n"
        "st1w {z1.s},  p7, [%[c0], #1, mul vl]\n"
        "st1w {z2.s},  p7, [%[c1]]\n"
        "st1w {z3.s},  p7, [%[c1], #1, mul vl]\n"
        "st1w {z4.s},  p7, [%[c2]]\n"
        "st1w {z5.s},  p7, [%[c2], #1, mul vl]\n"
        "st1w {z6.s},  p7, [%[c3]]\n"
        "st1w {z7.s},  p7, [%[c3], #1, mul vl]\n"
        "st1w {z8.s},  p7, [%[c4]]\n"
        "st1w {z9.s},  p7, [%[c4], #1, mul vl]\n"
        "st1w {z10.s}, p7, [%[c5]]\n"
        "st1w {z11.s}, p7, [%[c5], #1, mul vl]\n"
        "st1w {z12.s}, p7, [%[c6]]\n"
        "st1w {z13.s}, p7, [%[c6], #1, mul vl]\n"
        "st1w {z14.s}, p7, [%[c7]]\n"
        "st1w {z15.s}, p7, [%[c7], #1, mul vl]\n"
        : [b] "+r"(spm), [a0] "+r"(a0), [a1] "+r"(a1), [a2] "+r"(a2),
          [a3] "+r"(a3), [a4] "+r"(a4), [a5] "+r"(a5), [a6] "+r"(a6),
          [a7] "+r"(a7), [k] "+r"(kc)
        : [c0] "r"(c0), [c1] "r"(c1), [c2] "r"(c2), [c3] "r"(c3), [c4] "r"(c4),
          [c5] "r"(c5), [c6] "r"(c6), [c7] "r"(c7)
        : "p7", "z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8", "z9",
          "z10", "z11", "z12", "z13", "z14", "z15", "z16", "z17", "z18", "z19",
          "z20", "z21", "z22", "z23", "z24", "z25", "cc", "memory");
}

// Single-row tail: 2 C vectors (z0,z1) for rows that do not fill an 8-row tile.
static inline void spm_kernel_1x128(float *c0, const float *a0,
    ...
    for (int jc = col0; jc < col1; jc += kNc) {
        for (int pc = 0; pc < K; pc += opt.kc) {
            ...
            for (; i + kMr <= row1; i += kMr) {
                ...
                const float *ap[kMr];
                for (int r = 0; r < kMr; ++r) {
                    cp[r] = C + static_cast<std::size_t>(i + r) * N + jc;
                    ap[r] = A + static_cast<std::size_t>(i + r) * K + pc;
                }
                spm_kernel_8x128(cp[0], cp[1], cp[2], cp[3], cp[4], cp[5], cp[6],
                                 cp[7], ap[0], ap[1], ap[2], ap[3], ap[4], ap[5],
                                 ap[6], ap[7], spm_base,
            ...
            for (; i < row1; ++i) {
                ...
                spm_kernel_1x128(c0, a0, spm_base,
                                 static_cast<std::uint64_t>(opt.kc));
            }
#else
            for (int p = 0; p < opt.kc; ++p) {
...
static void *worker_entry(void *raw)
{
    ...
    std::vector<float> scratch(static_cast<std::size_t>(shared.opt.kc) * kNc);
    for (int rep = 0; rep < shared.total_reps; ++rep) {
        pthread_barrier_wait(&shared.barrier);
        run_rows(shared, row0, row1, col0, col1, scratch.data());
        pthread_barrier_wait(&shared.barrier);
    }
    return nullptr;
}
...
int main(int argc, char **argv)
{
...
#if defined(GEMM_SPM_NATIVE)
    if (svcntw() != kSpmWords) {
        std::fprintf(stderr, "requires VL=2048b (%d words/vector); got %u\n",
    ...
    const int PC = opt.grid_cols_override > 0 ? opt.grid_cols_override
                                              : grid_cols(opt.threads, panels);
    const int PR = opt.threads / PC;
    std::printf("gemm_spm_opt_mc2 threads=%d grid=%dx%d(PRxPC) m=%d k=%d n=%d "
                "kc=%d nc=%d mr=%d spm_way0=%d spm_base_set=%d warmup=%d repeat=%d "
                "verify=%s checksum=%.9f\n",
                opt.threads, PR, PC, opt.m, opt.k, opt.n, opt.kc, opt.nc, kMr,
                opt.spm_way0, opt.spm_base_set, opt.warmup, opt.repeat,
