...
struct Options {
    ...
    int mc = 128, kc = 256, warmup = 1, repeat = 1, verify = 16, pin = 1;
...
static void prepare_tile(Shared &sh, int tid) {
    ...
    const std::size_t ntiles = (cols + nt - 1) / nt;
    const std::size_t max_ab = (opt.mc + 7) / 8;
    t.bpack = Halfs(prepack_B_knm_size(opt.k, cols, opt.kc));
    ...
    (void)ntiles;
    prepack_B_knm(sh.b.ptr + t.col0, t.bpack.ptr, opt.k, cols, opt.kc, opt.n);
}
...
static void run_tile(const Shared &sh, Tile &t) {
    ...
    for (std::size_t k0 = 0, kt = 0; k0 < K; k0 += KC, ++kt) {
        ...
        for (std::size_t ntile = 0; ntile < ntiles; ++ntile) {
            std::size_t n0 = ntile * NT;
            std::size_t nc = std::min(NT, N - n0);
            const __fp16 *Bp = t.bpack.ptr + (kt * ntiles + ntile) * KC * NT;
            for (std::size_t m0 = 0; m0 < M; m0 += MC) {
                std::size_t mc = std::min(MC, M - m0);
                std::size_t ablocks = (mc + MT - 1) / MT;
                const __fp16 *Ap = t.acache.ptr + (m0 / MC) * max_ab * MT * KC;
                sve_interleaved_fp16_mla_8x3VL(Ap, Bp, t.cpanel.ptr, static_cast<int>(ablocks), 1, static_cast<int>(kc));
...
static int verify_samples(const Shared &sh) {
    ...
    for (int sample = 0; sample < opt.verify; ++sample) {
        ...
        for (int p = 0; p < opt.k; ++p)
            ref += static_cast<float>(sh.a.ptr[static_cast<std::size_t>(gi) * opt.k + p]) *
                   static_cast<float>(sh.b.ptr[static_cast<std::size_t>(p) * opt.n + gj]);
...
int main(int argc, char **argv) {
    ...
    int PC = opt.grid_cols_override > 0 ? opt.grid_cols_override : grid_cols(opt.threads, panels);
    std::printf("gemm_acl_fp16_blk_mc threads=%d grid=%dx%d mc=%d kc=%d nt=%zu m=%d k=%d n=%d verify=%s checksum=%.9f\n",
